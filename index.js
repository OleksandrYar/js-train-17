...
function extractTextInParentheses(str) {
  ...
  const arr = Array.from(match, (match) => match[1]);
...
function findWordOccurrences(str, word) {
  ...
  // Масив для зберігання індексів входжень слова
  const positions = [];

  // Використання властивості lastIndex для пошуку наступних входжень
  let match;

  // Цикл для пошуку всіх входжень слова
  while ((match = regex.exec(str)) !== null) {
    // Додавання індексу поточного входження слова у масив
    positions.push(match.index);

    // Оновлення lastIndex, щоб продовжити пошук з наступного символу
...
function findWord(str, word) {
  ...
  const searchedWord = new RegExp(word, "i");
  // Використання методу `search` для пошуку першого входження слова.
  const newWord = str.search(searchedWord);
  return newWord;
}
